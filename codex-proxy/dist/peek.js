...
var PROXY_ORIGIN = process.env.CODEX_PEEK_PROXY_ORIGIN || "http://127.0.0.1:3462";
...
var CODEX_HOME = process.env.CODEX_HOME || `${process.env.HOME || ""}/.codex`;
...
async function run(command, timeoutMs = 4000) {
...
async function tmuxCapture(url) {
...
function shell(command, timeoutMs = 4000) {
...
function sanitizeCommand(command) {
  return safeHomePath(command).replace(/--pseudonymization-salt-handle=\S+/g, "--pseudonymization-salt-handle=<redacted>").replace(/--field-trial-handle=\S+/g, "--field-trial-handle=<redacted>").replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/g, "Bearer <redacted>").replace(/(token|secret|key|password)=\S+/gi, "$1=<redacted>");
}
...
function processRole(command) {
  if (command.includes("codex app-server"))
    return "codex app-server";
  if (command.includes("Codex Helper") && command.includes("NetworkService"))
    return "Codex network service";
  if (command.includes("Codex Helper"))
    return "Codex helper/renderer";
  if (command.includes("/Contents/MacOS/Codex"))
    return "Codex Electron main";
  if (command.includes("/Resources/node_repl"))
    return "node_repl tool worker";
  if (command.includes("bun run bin/proxy.ts"))
    return "codex-proxy";
  if (command.includes("bun run bin/peek.ts"))
    return "codex-peek";
  if (command.includes("tmux new-session"))
    return "tmux supervisor";
  if (command.includes("Sparkle") || command.includes("Updater.app"))
    return "Codex updater";
  return "other";
}
function pidList(rows, role) {
  return rows.filter((row) => row.role === role && Number.isFinite(row.pid)).map((row) => row.pid);
}
function groupedEnvKeys() {
  const groups = {
    codex: [],
    openai: [],
    proxy: [],
  ...
  for (const key of Object.keys(process.env).sort()) {
    ...
    if (upper.includes("CODEX"))
      groups.codex.push(key);
    if (upper.includes("OPENAI") || upper.startsWith("OAI_"))
      groups.openai.push(key);
    if (upper.includes("PROXY"))
      groups.proxy.push(key);
    if (["PATH", "SHELL", "USER", "HOME", "PWD", "TMPDIR"].includes(upper))
      groups.runtime.push(key);
    if (upper.includes("HOST") || upper.includes("PORT") || upper.includes("SOCK"))
      groups.network.push(key);
    if (/(KEY|SECRET|TOKEN|AUTH|CERT|CRYPT|SIGN|HMAC)/.test(upper))
      groups.crypto_like.push(key);
  }
...
`)) {
    ...
    if (baseMatch)
      summary.openaiBaseUrl = baseMatch[1];
...
function codexFileSummary() {
  ...
  const topLevel = existsSync(CODEX_HOME) ? readdirSync(CODEX_HOME).sort().map((name) => {
...
function proxyLogSummary(limit) {
  ...
  for (const entry of api) {
    ...
    encryptedReplayed += input.filter((item) => item?.type === "reasoning" && typeof item.encrypted_content === "string").length;
  }
  return {
    logs: {
    ...
    paths,
    eventLayers,
    topEventTypes: Object.entries(eventTypes).map(([type, count]) => ({ type, count })).sort((a, b) => b.count - a.count).slice(0, 25),
    encryptedReasoning: {
...
async function nativeProbe(url) {
  const limit = Math.min(Number.parseInt(url.searchParams.get("limit") || "30000", 10) || 30000, 1e5);
  ...
  const [unix, openFiles, sessions] = await Promise.all([
    targetCsv ? shell(`lsof -nP -p ${targetCsv} -U 2>/dev/null | egrep 'COMMAND|codex-ipc|codex-browser-use|node_repl|->|\\.sock' || true`, 4000) : Promise.resolve({ ok: true, code: 0, stdout: "", stderr: "" }),
    appServerPids.length > 0 ? shell(`lsof -nP -p ${appServerPids.join(",")} 2>/dev/null | egrep 'COMMAND|\\.codex|sessions|sqlite|jsonl' || true`, 4000) : Promise.resolve({ ok: true, code: 0, stdout: "", stderr: "" }),
...
async function sqliteDbSummary(dbPath) {
  const tables = await sqliteJson(dbPath, "select name from sqlite_master where type='table' and name not like 'sqlite_%' order by name");
  const indexes = await sqliteJson(dbPath, "select name, tbl_name as tableName from sqlite_master where type='index' and name not like 'sqlite_autoindex_%' order by name");
...
function summarizeJsonlSession(path) {
  const st = statSync(path);
  const counts = {
    recordTypes: {},
    payloadTypes: {},
    ...
    try {
      ...
      if (typeof payload.type === "string") {
        ...
        if (type === "response_item")
          increment(counts.responseItemTypes, payload.type);
        if (type === "event_msg")
          increment(counts.eventMsgTypes, payload.type);
      }
...
async function persistenceMap() {
  const sessionRoot = `${CODEX_HOME}/sessions`;
  const sessionFiles = collectSessionFiles(sessionRoot).map((path) => ({ path, stat: statSync(path) })).sort((a, b) => b.stat.mtimeMs - a.stat.mtimeMs);
...
async function proxy(path, search) {
  try {
...
function stateChain(url) {
  const limit = Math.min(Number.parseInt(url.searchParams.get("limit") || "30000", 10) || 30000, 1e5);
  const events = readNdjsonTail(RAW_EVENT_LOG_PATH, limit);
  const api = readNdjsonTail(API_JSON_LOG_PATH, limit);
  const produced = new Map;
  const replayed = new Map;
  const promptCacheKeys = new Map;
  ...
  for (const entry of events) {
    ...
    if (event?.type === "response.output_item.done" && item?.type === "reasoning" && typeof item.encrypted_content === "string") {
  ...
  for (const entry of api) {
    if (entry.phase !== "request.forwarded" || entry.path !== "/responses")
      continue;
    const body = entry.body;
    const input = Array.isArray(body?.input) ? body.input : [];
    const encryptedItems = input.map((item, index) => ({ item, index })).filter(({ item }) => item?.type === "reasoning" && typeof item.encrypted_content === "string");
    requestRows.push({
      at: entry.at ?? null,
      requestId: entry.requestId ?? null,
  ...
  const matches = [...produced.values()].filter((item) => replayed.has(item.hash)).map((item) => ({
    ...item,
...
function wireLayerForEvent(type, event) {
  if (type.includes("reasoning_summary") || type.includes("reasoning_text"))
  ...
  if (item?.type === "reasoning")
    return typeof item.encrypted_content === "string" ? "state.encrypted_reasoning" : "analysis.item";
...
function summarizeSseEvent(entry) {
  const event = entry.event;
  ...
  if (response) {
    ...
    details.stopReason = response.stop_reason || response.incomplete_details?.reason || response.error?.code || null;
    if (typeof response.prompt_cache_key === "string") {
...
function isTimelineSse(type) {
  return !type.endsWith(".delta");
}
function requestRowFor(rows, requestId) {
...
function touchRequestRow(row, at) {
  if (typeof at !== "string")
    return;
  row.firstAt = row.firstAt || at;
  row.lastAt = at;
}
function wireTimeline(url) {
  const limit = Math.min(Number.parseInt(url.searchParams.get("limit") || "30000", 10) || 30000, 1e5);
  const requestIdFilter = url.searchParams.get("requestId") || "";
  const api = readNdjsonTail(API_JSON_LOG_PATH, limit).filter((entry) => !requestIdFilter || entry.requestId === requestIdFilter);
  const rows = new Map;
  const timeline = [];
  const phaseCounts = {};
  const layerCounts = {};
  for (const entry of api) {
    ...
    if (entry.phase === "request.received" || entry.phase === "request.forwarded") {
      const bodySummary = summarizeRequestBody(entry.body);
      ...
      timeline.push({
        at: entry.at ?? null,
        requestId,
        transport: entry.phase === "request.received" ? "app-server -> proxy" : "proxy -> backend",
        layer: "request.body",
        phase: entry.phase,
    ...
    if (typeof event?.delta === "string") {
      if (type.includes("output_text"))
        row.channelByteCounts.commentaryVisible += event.delta.length;
      if (type.includes("reasoning"))
        row.channelByteCounts.analysisVisible += event.delta.length;
      if (type.includes("function_call_arguments"))
        row.channelByteCounts.toolArguments += event.delta.length;
      if (type.includes("custom_tool_call_input"))
        row.channelByteCounts.customToolInput += event.delta.length;
    }
    ...
    if ((event?.item?.type === "function_call" || event?.item?.type === "custom_tool_call") && (type === "response.output_item.added" || type === "response.output_item.done")) {
      row.toolItems.push({
        at: entry.at ?? null,
        event: type,
...
async function channelGraph(url) {
  const limit = Math.min(Number.parseInt(url.searchParams.get("limit") || "30000", 10) || 30000, 1e5);
  const events = readNdjsonTail(RAW_EVENT_LOG_PATH, limit);
  const api = readNdjsonTail(API_JSON_LOG_PATH, limit);
  ...
  for (const entry of events) {
    ...
    if (typeof entry.event?.response?.model === "string")
      models.add(entry.event.response.model);
  ...
  for (const entry of api) {
    const input = Array.isArray(entry.body?.input) ? entry.body.input : [];
    replayedEncryptedReasoningItems += input.filter((item) => item?.type === "reasoning" && typeof item.encrypted_content === "string").length;
  }
  const edges = [
    {
...
`);
  return json({
    ...
    eventChannels: {
      groups: Object.entries(eventGroups).map(([group, count]) => ({ group, count })).sort((a, b) => b.count - a.count),
      types: Object.entries(eventTypes).map(([type, count]) => ({ type, count })).sort((a, b) => b.count - a.count).slice(0, 80),
    ...
    apiChannels: {
      phases: Object.entries(apiPhases).map(([phase, count]) => ({ phase, count })).sort((a, b) => b.count - a.count),
      paths: Object.entries(apiPaths).map(([path, count]) => ({ path, count })).sort((a, b) => b.count - a.count)
    },
    liveLocalEdges: {
...
function startPeekServer(options = {}) {
  ...
  const server = Bun.serve({
    ...
    fetch(req) {
      ...
      if (url.pathname === "/api/tmux/sessions")
        return tmuxSessions();
      if (url.pathname === "/api/tmux/capture")
        return tmuxCapture(url);
      if (url.pathname === "/api/codex/processes")
        return codexProcesses();
      if (url.pathname === "/api/ports")
        return ports();
      if (url.pathname === "/api/native/probe")
        return nativeProbe(url);
      if (url.pathname === "/api/persistence/map")
        return persistenceMap();
      if (url.pathname === "/api/proxy/health")
        return proxy("/health", "");
      if (url.pathname === "/api/proxy/transcript")
        return proxy("/debug/transcript", url.search);
      if (url.pathname === "/api/state/chain")
        return stateChain(url);
      if (url.pathname === "/api/wire/timeline")
        return wireTimeline(url);
      if (url.pathname === "/api/channel/graph")
        return channelGraph(url);
